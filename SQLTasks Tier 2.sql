...
CASE WHEN m.recommendedby <> 0 THEN CONCAT(M.firstname,' ',M.surname)
ELSE 'N/A' END AS Recommended_By 
FROM Members AS m
INNER JOIN Members AS M
ON m.recommendedby = M.memid
WHERE m.memid <> 0
ORDER BY m.surname, m.firstname, M.surname, M.firstname;
